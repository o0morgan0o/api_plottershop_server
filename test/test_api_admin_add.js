...
describe('testing add delete update item', () => {
    ...
    it('ADD item at /admin/additem', async () => {
        ...
        formDataNewItem.append('file_main', fs.createReadStream('./test/placeholder.png'))
        const resAdd = await Axios.post(`${BASE_URL}/api/v1/admin/additem`, formDataNewItem, { headers: getHeaderWithCookie(formDataNewItem, cookie) })
        ...
        const resGetAfter = await Axios.get(`${BASE_URL}/api/v1/item/${id}`)
    ...
    it('DELETE at /admin/deleteitem', async () => {
        ...
        const resGet = await Axios.get(`${BASE_URL}/api/v1/items`)
        ...
        const resDel = await Axios.delete(`${BASE_URL}/api/v1/admin/deleteitem/${choosenItem.id}`, {
            params: { itemId: choosenItem.id },
            headers: { Cookie: cookie }
        })
        expect(resDel).to.have.status(200)
        expect(resDel.data.status).to.be.equal('success')
        const resGetAfter = await Axios.get(`${BASE_URL}/api/v1/item/${choosenItem.id}`)
    ...
    it('try to DELETE item without login', async function () {
        ...
        const choosenItem = items[index]
        const resDel = await Axios.delete(`${BASE_URL}/api/v1/admin/deleteitem/${choosenItem.id}`, { params: { itemId: choosenItem.id } })
        expect(resDel).to.have.status(200)
